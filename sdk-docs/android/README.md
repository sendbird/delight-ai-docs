...
{% hint style="warning" %}
**Important:** When using Manual Session, you **must** implement a `SessionHandler` to handle session expiration and token refresh. See the [Handle session expiration](#handle-session-expiration) section below for implementation details.
{% endhint %}
...
{% hint style="info" %}
Replace `YOUR_AI_AGENT_ID` with your AI agent ID which you can obtain from the Delight AI dashboard. To learn how do to so, refer to the [prerequisites](#prerequisites) section.
{% endhint %}
